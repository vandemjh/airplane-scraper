...
def findURL(string):
    if isinstance(string, str):
        return string[
            string.index("http") : len(string) - 1 - (string[::-1].index('"'))
        ]
    ...
    try:  # Handles improper links
        ...
        if page.status_code == 200:
            for space in str(page.content).split(" "):
                if (
                    URL in space or ALLOW_OUTSIDE_URLS
                ):  # and URL in space for only on this domain
...
stopPoint = (
    50 if len(sortedValues) > 50 else len(sortedValues)
)  # math.floor(len(valueRange) * .1)
...
axesSettings = [0.11, 0.11, 0.8, 0.8]
...
ax.set_title(
    "Number of occurances by code fragement ("
    + str(
        "<.1" if math.floor(stopPercent * 100) == 0 else math.floor(stopPercent * 100)
    )
    + "% of values)"
)
# plt.show()
...
print(
    'Most used code fragement: "'
    + str(list(toReturn.keys())[list(toReturn.values()).index(sortedValues[0])])
    + '" used '
    + str(sortedValues[0])
    + " times!"
)
